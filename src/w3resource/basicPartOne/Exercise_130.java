...
// #######################################################################################################
// #                                                                                                     #
// #    Program Purpose: Finds the maximum depth of a given binary tree.                                 #
...
public class Exercise_130 {
    ...
    private static class BinaryTree<T extends Comparable<T>> {
        ...
        private int maxDepth(Node<T> rootNode) {
            if (Objects.isNull(rootNode))
                return 0;
            int leftDepth = this.maxDepth(rootNode.getLeftTree());
            int rightDepth = this.maxDepth(rootNode.getRightTree());
    ...
    public static void main(String[] args) {
        int[] mainData = {55, 21, 80, 9, 29, 76, 91};
        final int[] i = {0};

        BinaryTree<Integer> mainBinaryTree = new BinaryTree<>(mainData[0]);
        IntStream.of(mainData).forEach(val -> mainBinaryTree.addItem(mainData[i[0]++]));

        System.out.printf("Traversed tree data: %s\n", mainBinaryTree.preOrderTraversal());
        System.out.printf("Maximum depth of tree: %s\n", mainBinaryTree.maxDepthFromRoot());
    }
}
